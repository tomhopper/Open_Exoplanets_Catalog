...
density_colors <- matrix(data = rep(NA, times = ((length(density_values))*4)), nrow = (length(density_values)))
...
density_colors[,1] <- colorRampPalette(c(density_colors_temp[1], density_colors_temp[length(density_colors_temp)]))(length(density_values))
density_colors[,ncol(density_colors)] <- rep("#FFFFFF", times = nrow(density_colors))
for(i in 1:nrow(density_colors)) {
  density_colors[i,] <- colorRampPalette(c(density_colors[i,1], density_colors[i,ncol(density_colors)]))(ncol(density_colors))
}
...
planets_p <- oec_df %>% 
  ...
  stat_function(aes(), fun = function(x) {log10(x^(1/3))}, color = density_colors[3,1], alpha = 0.25) +
  ...
  scale_color_gradientn(guide = FALSE, labels = scales::comma, colours = density_colors[,1], values = density_values, name = expression(paste("Density,"  %*% rho[jup]))) +
  coord_equal() +
